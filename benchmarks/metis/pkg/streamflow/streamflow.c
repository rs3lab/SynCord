...
static __thread counting_queue_t
    local_inactive_pageblocks[PAGEBLOCK_SIZE_CLASSES];
static counting_lf_lifo_queue_t
    global_partial_pageblocks[OBJECT_SIZE_CLASSES];
static counting_lf_lifo_queue_t
    global_free_pageblocks[PAGEBLOCK_SIZE_CLASSES];

static inline unsigned int quick_log2(unsigned int x);
static inline int max(int a, int b);
static int compute_size_class(size_t size);
static inline int reverse_size_class(size_t size_class);

/* Radix tree operations. */
static void radix_extract(void *object, void **ptr, size_t * size,
			  short *is_large);
static void radix_register(void *start, int num_pages, void *ptr,
			   size_t size, short object_type);
...
static inline int find_index(superpage_t * super, page_chunk_t * chunk,
			     int order);
static inline void *find_buddy(superpage_t * super, page_chunk_t * chunk,
			       int order);
static inline int find_bit_index(superpage_t * super, page_chunk_t * chunk,
				 int order);
...
static inline void register_pages(void *start, int num_pages, void *ptr,
				  size_t size, short object_type);
...
static pageblock_t *remove_global_pageblocks(int class_index,
					     int pageblock_size);
...
static inline void headerize_object(void **object, void *ptr, size_t size,
				    short object_type);
...
static inline void local_free(void *object, pageblock_t * pageblock,
			      heap_t * my_heap);
static void remote_free(void *object, pageblock_t * pageblock,
			heap_t * my_heap);
static void adopt_pageblock(void *object, pageblock_t * pageblock,
			    heap_t * my_heap);
...
#define mremap u_mremap
#elif defined(HUGETLB)

#define mmap hugemmap
#define munmap hugemunmap
#define PREALLOC
#ifdef PREALLOC
//#define PREFAULT
#endif

// Streamflow mapps a segment of SPAGE_SM MB per core at once. However, it
// may still call mmaps to allocate few pages to hold metadata.
...
static void *
hugemmap(void *addr, size_t len, int prot, int flags, int fd, off_t offset)
{
#undef mmap
#define ROUNDUP(a,b) (((a) % (b) == 0) ? (a) : (((a) / (b) + 1) * (b)))
    size_t pgsize = HPAGE_SK * 1024;
    static __thread int hugefd = -1;
    static __thread off_t fd_off = 0;
    if (hugefd < 0) {
	char path[256];
    ...
    static __thread char *basep = NULL;
    static __thread size_t segoff = 0;
    assert(offset == 0);
    if (len >= segsize) {
	// Size/offset must be aligned at page boundary, which is 2M for hugetlb
    ...
    if (basep == NULL || segsize - segoff < len) {
	basep =
...
static void *
hybridmmap(void *addr, size_t len, int prot, int flags, int fd, off_t offset)
{
#undef mmap
#define ROUNDUP(a,b) (((a) % (b) == 0) ? (a) : (((a) / (b) + 1) * (b)))
    if (len >= (HPAGE_SK * 1024)) {
	size_t pgsize = HPAGE_SK * 1024;
	len = ROUNDUP(len, pgsize);
#ifdef PREALLOC
	static __thread char *basep = NULL;
	static __thread size_t maxlen = 0;
	static __thread volatile unsigned int hused = 0;
	if (!basep) {
	    char *ptr;
	    uint64_t s, e;
	    maxlen = 400 * 1024 * 1024;
	    if (thread_id == 0) {
		maxlen = 3000 * 1024 * 1024;
	    }
...
#ifdef PREFAULT
	    s = read_tsc();
	    for (ptr = basep; ptr < basep + maxlen; ptr += 4096) {
		ptr[0] = 0;
	    }
	    e = read_tsc();
	    printf
		("base mapped %p, requested %ld, %ld cycles per page, %ld ms\n",
		 basep, len, (e - s) / (maxlen / 4096),
		 (e - s) * 1000 / (2UL * 1024 * 1024 * 1024));
#else
	    printf("base mapped %p, requested %ld\n", basep, len);
#endif
	}
	off_t alloced =
...
static void *
hacked_mremap(void *old_address, size_t old_size, size_t new_size, int flags)
{
    if (old_size >= new_size)
	return old_address;
    void *newp = mmap(NULL, new_size, PROT_READ | PROT_WRITE,
		      MAP_ANONYMOUS | MAP_PRIVATE,
		      -1, 0);
    memcpy(newp, old_address, old_size);
    return newp;
}
...
static inline void
memory_add32(volatile unsigned int *address, int value)
{
#ifdef MEMORY
...
static inline void
memory_add64(volatile unsigned long long *address, unsigned long long value)
{
#ifdef MEMORY
...
static int
compute_size_class(size_t size)
{
...
static inline int
malloc_compute_size_class(size_t size)
{
#ifdef PROFILE
...
static inline int
free_compute_size_class(size_t size)
{
#ifdef PROFILE
...
static inline int
reverse_size_class(size_t size_class)
{
    return reverse[size_class];
}

static inline radix_interior_t *
radix_interior_alloc(void)
{
    void *node = page_alloc(sizeof(radix_interior_t));
    return (radix_interior_t *) node;
}

static inline radix_leaf_t *
radix_leaf_alloc(void)
{
...
static inline void
radix_interior_free(radix_interior_t * node)
{
    //          printf("freeing radix_interior\n");
    page_free(node, sizeof(radix_interior_t));
}

static inline void
radix_leaf_free(radix_leaf_t * node)
{
    //    printf("freeing radix_leaf\n");
    page_free(node, sizeof(radix_leaf_t));
}

static void
radix_register(void *start, int num_pages, void *ptr, size_t size,
	       short object_type)
{
	...
	unsigned long level1 =
	    page >> (RADIX_INTERIOR_BITS + RADIX_LEAF_BITS);
	unsigned long level2 =
	    (page >> RADIX_LEAF_BITS) & (RADIX_INTERIOR_SIZE - 1);
	...
	if (radix_root->prefixes[level1] == NULL) {
		...
		(&radix_root->prefixes[level1], NULL, temp_interior)) {
	...
	if (radix_root->prefixes[level1]->prefixes[level2] == NULL) {
	    ...
	    if (!compare_and_swap_ptr
		(&radix_root->prefixes[level1]->prefixes[level2], NULL,
		 temp_leaf)) {
	...
	switch (object_type) {
	case OBJECT_SMALL:
	    record.pageblock = (unsigned long) ptr >> PAGE_BITS;
	    break;

	case OBJECT_MEDIUM:
	    record.sph = (unsigned long) ptr >> SUPERPAGE_BITS;
	    record.log_size = log_size;
	    break;

	case OBJECT_LARGE:
	    record.size = (unsigned long) size;
	    break;
	}

	((radix_leaf_t *) radix_root->prefixes[level1]->prefixes[level2])->
	    values[level3] = record;
...
static void
radix_extract(void *object, void **ptr, size_t * size, short *object_type)
{
    ...
    *object_type = record.object_type;
    switch (*object_type) {
    case OBJECT_SMALL:
	*ptr = (void *) (unsigned long long) (record.pageblock << PAGE_BITS);
	break;
    case OBJECT_MEDIUM:
	*ptr = (void *) (unsigned long long) (record.sph << SUPERPAGE_BITS);
...
static inline int
find_index(superpage_t * super, page_chunk_t * chunk, int order)
{
    return ((unsigned long) chunk -
	    (unsigned long) super->page_pool) / (PAGE_SIZE * (1 << order));
}

static inline void *
find_buddy(superpage_t * super, page_chunk_t * chunk, int order)
{
    void *buddy;
    ...
    if (i % 2 == 0) {
	buddy =
	    (superpage_t *) ((unsigned long) chunk +
			     ((1 << order) * PAGE_SIZE));
    } else {
	buddy =
	    (superpage_t *) ((unsigned long) chunk -
...
static inline int
find_bit_index(superpage_t * super, page_chunk_t * chunk, int order)
{
    int i = find_index(super, chunk, order);
...
static void *
buddy_alloc(superpage_t * super, size_t size)
{
	...
	if (super->buddy[curr_order].free_list.head != NULL) {
	    ...
	    __change_bit(find_bit_index(super, chunk, curr_order),
			 (unsigned long *) super->buddy[curr_order].bitmap);
	    break;
	}
    }
...
static void
buddy_free(superpage_t * super, void *start, size_t length)
{
	...
	if (!__test_and_change_bit
	    (find_bit_index(super, chunk, curr_order),
	     (unsigned long *) super->buddy[curr_order].bitmap)) {
	    break;
	}
	...
	double_list_insert_front(chunk, &super->buddy[curr_order].free_list);
	if (curr_order > super->largest_free_order
	    || super->largest_free_order > BUDDY_ORDER_MAX) {
	    super->largest_free_order = curr_order;
	}
    } else {
	//Yandong Mao: bug fixed
	//page_free(chunk, SUPERPAGE_SIZE);
	page_free(super->page_pool, SUPERPAGE_SIZE);
...
static void *
quickie_alloc(quickieblock_t * quickie, size_t object_size)
{
    void *object;
...
static inline void
quickie_free(quickieblock_t * quickie, void *object)
{
...
static void
get_free_superpage(superpage_t ** super, size_t size)
{
    ...
    *super = NULL;
	...
	do {
		...
		&& (1 << ((superpage_t *) curr)->largest_free_order) >=
		(size / PAGE_SIZE)) {
    ...
    if (*super == NULL) {
	*super =
	    (superpage_t *) quickie_alloc(&sph_pageblocks,
					  sizeof(superpage_t));
	...
	for (order = 0; order < BUDDY_ORDER_MAX - 1; ++order) {
	    byte += max(sizeof(unsigned long), (int)
			ceil(((double) PAGES_PER_SUPERPAGE /
			      ((1 << order) * 8 * 2))));
	...
	double_list_insert_front((*super)->page_pool,
				 &((*super)->buddy[BUDDY_ORDER_MAX - 1].
				   free_list));
	(*super)->largest_free_order = BUDDY_ORDER_MAX - 1;
...
static void
superunmap(void *start, size_t length)
{
...
static inline void
register_pages(void *start, int num_pages, void *ptr, size_t size,
	       short object_type)
{
#ifdef RADIX_TREE
	...
	switch (object_type) {
	case OBJECT_SMALL:
	    bibop[page / PAGE_SIZE].pageblock =
		(unsigned long) ptr >> PAGE_BITS;
	    break;

	case OBJECT_MEDIUM:
	    bibop[page / PAGE_SIZE].sph =
		(unsigned long) ptr >> SUPERPAGE_BITS;
	    bibop[page / PAGE_SIZE].log_size = quick_log2(size / PAGE_SIZE);
	    break;

	case OBJECT_LARGE:
	    bibop[page / PAGE_SIZE].size = size;
	    break;
	}
	page += PAGE_SIZE;
    }
...
page_alloc(size_t size)
{
    ...
    addr =
	mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE,
	     -1, 0);
...
static inline void
page_free(void *start, size_t length)
{
    munmap(start, length);
}
...
static void *
medium_or_large_alloc(size_t size)
{
    void *mem;
	...
	register_pages(mem, 1, ((pageblock_t *) mem)->sph, size,
		       OBJECT_MEDIUM);
	headerize_object(&mem, ((pageblock_t *) mem)->sph, size,
			 OBJECT_MEDIUM);
...
static void
insert_global_free_pageblocks(pageblock_t * pageblock)
{
    int size_index =
	quick_log2((pageblock->mem_pool_size +
		    (unsigned long) pageblock->mem_pool -
		    (unsigned long) pageblock) / PAGE_SIZE) -
	...
	//Yandong Mao: fixed
	superunmap(pageblock,
		   pageblock->mem_pool_size +
		   (unsigned long) pageblock->mem_pool -
		   (unsigned long) pageblock);
...
static pageblock_t *
remove_global_pageblocks(int class_index, int pageblock_size)
{
    pageblock_t *pageblock = (pageblock_t *)
	lf_lifo_dequeue(&global_partial_pageblocks[class_index].queue);
    if (pageblock) {
	atmc_add32(&global_partial_pageblocks[class_index].count, -1);
    } else {
	int size_index =
	    quick_log2(pageblock_size / PAGE_SIZE) -
	    quick_log2(MIN_PAGEBLOCK_SIZE / PAGE_SIZE);

	pageblock = (pageblock_t *)
	    lf_lifo_dequeue(&global_free_pageblocks[size_index].queue);
	if (pageblock) {
	    atmc_add32(&global_free_pageblocks[size_index].count, -1);
	}
    }
...
static void
double_list_insert_front(void *new_node, double_list_t * list)
{
...
static void
double_list_rotate_back(double_list_t * list)
{
    double_list_elem_t *old_head = list->head;
...
static void
double_list_remove(void *node, double_list_t * list)
{
...
static void
garbage_collect(pageblock_t * collectee)
{
    unsigned int chain;
    queue_node_t header;
    ...
    chain = lf_lifo_chain_dequeue_nABA32((unsigned int *)
					 &(collectee->garbage_head));
...
void
streamflow_thread_finalize(void)
{
#if 0
    int i;
	...
	if (pageblock) {
		...
		if (pageblock->num_free_objects ==
		    pageblock->mem_pool_size / pageblock->object_size) {
		    insert_global_free_pageblocks(pageblock);
		} else {
		    if (pageblock->num_free_objects > 0
			|| pageblock->garbage_head.next != 0) {
			...
			if (!compare_and_swap64
			    (&(pageblock->together), with_id, no_id)) {
			    insert_global_partial_pageblocks(pageblock, i);
			}
		    }
		}
	...
	while ((pageblock = (pageblock_t *)
		seq_lifo_dequeue(&local_inactive_pageblocks[i].queue)) !=
	       NULL) {
	    insert_global_free_pageblocks(pageblock);
	}
    }
...
static inline int
compute_pageblock_size(int index)
{
    /* Make sure that the suggestion is a page multiple. */
    unsigned int suggestion =
	ceil((double) (reverse_size_class(index) * OBJECTS_PER_PAGEBLOCK) /
	     PAGE_SIZE) * PAGE_SIZE;
 ...
 * with a free object. */
static pageblock_t *
get_free_pageblock(heap_t * heap, int index)
{
    ...
    pageblock = (pageblock_t *)
	seq_lifo_dequeue(&local_inactive_pageblocks[size_index].queue);
	...
	lf_lifo_queue_init_nABA32((unsigned int *)
				  &(pageblock->garbage_head));
	pageblock->freed = 0;
	pageblock->unallocated = 1;
	pageblock->object_size = reverse_size_class(index);
	pageblock->mem_pool =
	    (char *) ((unsigned long) pageblock +
		      (unsigned long) ceil((double) sizeof(pageblock_t) /
					   CACHE_LINE_SIZE) *
		      CACHE_LINE_SIZE);
	pageblock->mem_pool_size =
	    pageblock_size - (unsigned long) pageblock->mem_pool +
	    (unsigned long) pageblock;
...
{
    fprintf(stderr,
	    "totsmall %u totmedium %u totlarge %u szsmall %llu szmedium %llu szlarge %llu frees %u remote %u adopt %u\n",
	    num_total_small, num_total_medium, num_total_large,
	    size_total_small, size_total_medium, size_total_large, num_frees,
	    num_remote_frees, num_adoptions);
    fflush(stderr);
}

void
cycle_counts(int sig)
{
#ifdef PROFILE
...
static inline void
headerize_object(void **object, void *ptr, size_t size, short object_type)
{
#ifdef HEADERS
    ...
    switch (object_type) {
    case OBJECT_SMALL:
	((header_t *) * object)->pageblock = (unsigned long) ptr >> PAGE_BITS;
	break;

    case OBJECT_MEDIUM:
	((header_t *) * object)->sph = (unsigned long) ptr >> SUPERPAGE_BITS;
	((header_t *) * object)->log_size = quick_log2(size / PAGE_SIZE);
	break;

    case OBJECT_LARGE:
	((header_t *) * object)->size = size;
	break;
    }
...
static void *
sf_malloc(size_t requested_size)
{
    int index;
    void *pointer = NULL;
    pageblock_t *pageblock;
	...
	pointer =
	    pageblock->mem_pool + (pageblock->freed -
				   1) * pageblock->object_size;
	pageblock->freed = ((queue_node_t *) pointer)->next;
    } else {
	pointer =
	    pageblock->mem_pool + (pageblock->unallocated -
				   1) * pageblock->object_size;
	pageblock->unallocated++;

	if (pageblock->unallocated >
	    pageblock->mem_pool_size / pageblock->object_size) {
...
static inline void
local_free(void *object, pageblock_t * pageblock, heap_t * my_heap)
{
#ifdef PROFILE
	...
	if (local_inactive_pageblocks[size_index].count <
	    MAX_PRIVATE_INACTIVE) {
	    seq_lifo_enqueue(&local_inactive_pageblocks[size_index].queue,
			     pageblock);
	    local_inactive_pageblocks[size_index].count++;
	} else {
	    insert_global_free_pageblocks(pageblock);
    ...
    else if (pageblock != (pageblock_t *) my_heap->active_pageblocks.head
	     && pageblock->num_free_objects > 1) {
	double_list_remove(pageblock, &my_heap->active_pageblocks);
...
static void
adopt_pageblock(void *object, pageblock_t * pageblock, heap_t * my_heap)
{
#ifdef PROFILE
...
static void
remote_free(void *object, pageblock_t * pageblock, heap_t * my_heap)
{
...
static inline void
object_extract(void **object, void **ptr, size_t * size, short *object_type)
{
#ifdef PROFILE
    ...
    *object_type = ((header_t *) * object)->object_type;
    switch (*object_type) {
    case OBJECT_SMALL:
	*ptr = (void *) (((header_t *) * object)->pageblock << PAGE_BITS);
	break;
    case OBJECT_MEDIUM:
	*ptr = (void *) (((header_t *) * object)->sph << SUPERPAGE_BITS);
    ...
    *object_type = bibop[page / PAGE_SIZE].object_type;
    ...
    switch (*object_type) {
    case OBJECT_SMALL:
	*ptr = (void *) (bibop[page / PAGE_SIZE].pageblock << PAGE_BITS);
	break;
    case OBJECT_MEDIUM:
	*ptr = (void *) (bibop[page / PAGE_SIZE].sph << SUPERPAGE_BITS);
	*size = (size_t) (1 << bibop[page / PAGE_SIZE].log_size) * PAGE_SIZE;
	break;
    case OBJECT_LARGE:
	*size = (size_t) (bibop[page / PAGE_SIZE].size);
	break;
    }
#endif
#ifdef PROFILE
...
sf_free(void *object)
{
    ...
    heap_t *my_heap =
	&local_heap[free_compute_size_class(pageblock->object_size)];
...
realloc(void *object, size_t size)
{
	...
	if (size <= SUPERPAGE_SIZE) {
	    ...
	    memcpy(new_object, original_object, size);
	...
	/* Remains large, but has to shrink */
	    ...
	    // Yandong Mao, hacked remap
	    new_object = hacked_mremap(object, old_size, size, 0);
	    //                  printf("streaflow mremap %p (%lx->%lx)\n", new_object, old_size, size);
	    if (new_object == MAP_FAILED) {
		fprintf(stderr, "realloc(): first mremap failed at address %p"
	...
	/* Object has to expand */
	    ...
	    new_object =
		hacked_mremap(object, old_size, size, MREMAP_MAYMOVE);
	    //          printf("streaflow mremap %p (%lx->%lx)\n", new_object, old_size, size);
	    if (new_object == MAP_FAILED) {
		fprintf(stderr,
			"realloc(): second mremap failed at address %p"
			" for %zu -> %zu\n", object, old_size, size);
		exit(1);
	    }
	}
	...
	/* Object will remain medium */
	    ...
	    register_pages(new_object, 1, ((pageblock_t *) new_object)->sph,
			   super_size, OBJECT_MEDIUM);
	    headerize_object(&new_object, ((pageblock_t *) new_object)->sph,
			     super_size, OBJECT_MEDIUM);
	}
	...
	if (old_size < size) {
	    memcpy(new_object, original_object, old_size);
	} else {
	    memcpy(new_object, original_object, size);
	}
	...
	&& compute_size_class(old_size) == compute_size_class(size)) {
	return original_object;
    }
