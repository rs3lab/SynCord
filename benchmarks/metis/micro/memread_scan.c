...
static int remote_only;
enum { localbufsize = 100 * 1024 * 1024 };

static int lcpu_to_pcpu[] = {0, 6, 12, 18, 24, 30, 36, 42};
...
worker(void *arg)
{
	...
	for (int j = 0; j < ncores; j++) {
	    if (remote_only && (j % 8 == c % 8))
		continue;
	    void *src = gstate->state[j].v.buf + (read_tsc() % (chunksize - scan_size / nsplits));
	    memread(src, scan_size / nsplits);
	}
    }
	...
	for (int i = 0; i < ncores; i++)
        ...
        double mbytes_accessed = nscans * ncores * scan_size / (1024 * 1024);
...
main(int argc, char **argv)
{
    ...
    if (argc < 3) {
	printf("Usage: <%s> number-cores remote-only\n", argv[0]);
	exit(EXIT_FAILURE);
    }
    ncores = atoi(argv[1]);
    remote_only = atoi(argv[2]);
