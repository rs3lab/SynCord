...
static void worker_main(void *arg)
{
	...
	int err = 0;
	int cpu = 0;
	int total_cpu = bench->ncpu;

	printf("worker id %d, total_cpu %d, CORE_PER_CHIP %d\n", worker->id, total_cpu, CORE_PER_CHIP);
	if (total_cpu >= CORE_PER_CHIP) {
		cpu = worker->id;
		setaffinity(worker->id);
	}
	else {
		cpu = (worker->id == 0 || worker->id < total_cpu / 2)?
			worker->id :
			(worker->id - total_cpu / 2) + CORE_PER_CHIP/2;
		setaffinity(cpu);
	}

	worker->fast = (cpu % CORE_PER_CHIP) < (CORE_PER_CHIP/ 2);
	printf("cpu: %d, worker->fast: %d\n", cpu, worker->fast);
...
void report_bench(struct bench *bench, FILE *out)
{
	...
	double fast_works = 0.0, slow_works = 0.0;
	double   avg_secs;
	...
	fprintf(out, "# ncpu secs works works/sec %s fast_works/sec, slow_works/sec\n", profile_name);
	fprintf(out, "%d %f %f %f %f %f %s\n",
			n_fg_cpu, avg_secs, total_works, total_works/avg_secs, fast_works/avg_secs, slow_works/avg_secs, profile_data);
